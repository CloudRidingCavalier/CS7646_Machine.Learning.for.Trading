...
class StrategyLearner(object):                                                                
    ...
    def generateX_train(self, prices, syms):
        ...
        ema20 = EMA(prices, syms, 20).fillna(method='bfill')
        ema50 = EMA(prices, syms, 50).fillna(method='bfill')
        ...
        df = np.zeros((len(prices) - self.RETURN, 11))
        ...
        for i in range(0, len(prices) - self.RETURN):
            ...
            df[i][6] = ema20.iloc[i]
            df[i][7] = ema50.iloc[i]
            df[i][8] = ema50.iloc[i] - ema20.iloc[i]
            df[i][9] = bba.iloc[i]
            df[i][10] = prices.iloc[i + self.RETURN]
    ...
    def generateX_test(self, prices, syms):
        ...
        for i in range(0, len(prices) - self.RETURN):
            ...
            df[i][2] = sma20.iloc[i]
            df[i][3] = sma50.iloc[i]
            df[i][4] = sma50.iloc[i] - sma20.iloc[i]
            df[i][5] = ema20.iloc[i]
            df[i][6] = ema50.iloc[i]
            df[i][7] = ema50.iloc[i] - ema20.iloc[i]
            df[i][8] = bba.iloc[i]
