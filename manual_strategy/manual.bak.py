...
def testPolicy(symbol="JPM", sd=dt.datetime(2008,1,1), ed=dt.datetime(2009,12,31), sv=100000):
    ...
    [SMA_20, SMA_20_std, BB_upper_band, BB_lower_band, BB_value] = BollingerBands(prices, symbol, 20)
    ...
    indicators_df = indicators_df.fillna(method='ffill')
    ...
    signal_df = pd.DataFrame(index=prices.index, columns=['SMA', 'BB', 'Momentum'])
...
def main(start_date=dt.datetime(2008,1,1), end_date=dt.datetime(2009,12,31)):
    ...
    portfolio_df = pd.DataFrame(index=Manual_portval.index, columns=['Theoretically Optimal Strategy', 'Benchmark'])
    
    portfolio_df['Theoretically Optimal Strategy'] = Manual_portval / Manual_portval[0]
